...
def dashboard():
    ...
    if edit_form.validate_on_submit():
        ...
        if 'photo' in request.files:
            ...
            path = f'photos/{filename}'
        new_blog = BlogPost( title = form.title.data, content = form.content.data, category = form.category.data, likes = 0, dislikes=0, rating = 0, image = path, time = datetime.utcnow().strftime("%H:%M"), author=current_user)
...
def article(title):
    ...
    if form.validate_on_submit():
        ...
        new_comment = Comment(content=form.content.data, likes=0, dislikes=0, time=datetime.utcnow().strftime("%H:%M"), blog=blog, author=current_user)
